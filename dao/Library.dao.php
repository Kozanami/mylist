...
class DaoLibrary {
	...
	public function read($userid,$firstEntry,$lastEntry) {
        $datas = DB::request('SELECT * FROM library WHERE userid=?  ORDER BY id DESC LIMIT '.$firstEntry.', '.$lastEntry.'',array($userid));
        if (!empty($datas)) {
    ...
    public function countLibrary($userid){
